...
exports.fillConcentricIndices = fillConcentricIndices;
exports.fillConcentricToBuffer = fillConcentricToBuffer;
const fillMath_1 = require("./fillMath");
const sortAscending = (a, b) => a - b;
const INF = 1e12;
const makeNoiseSampler = (seed) => {
    const seedInt = Math.floor(seed * 1000000) | 0;
    return (x, y) => {
...
const clampIndex = (value) => Math.max(1, Math.min(255, value | 0));
const withinBounds = (x, y, bbox) => {
    return (x >= bbox.minX &&
        x < bbox.minX + bbox.width &&
        y >= bbox.minY &&
        y < bbox.minY + bbox.height);
};
const buildMaskAndRowSpans = (vertices, bbox, width, height) => {
    const rowSpans = Array.from({ length: height }, () => []);
    if (!vertices.length) {
        return { rowSpans, mask: new Uint8Array(0), maskWidth: 0, maskHeight: 0, coverage: null };
    }
    let minRow = height;
    let maxRow = -1;
...
const computeDistanceField = (mask, width, height) => {
...
async function fillConcentricIndices(params, hooks) {
    ...
    const { rowSpans, mask, maskWidth, maskHeight, coverage } = buildMaskAndRowSpans(params.vertices, params.bbox, width, height);
    ...
    const requestYield = async (iteration) => {
        if (maybeYield) {
            await maybeYield(iteration);
        }
    };
    const clampNormalized = (value) => {
        return (0, fillMath_1.applyEdgePadding)(value);
    };
    ...
    const sampleDistance = (worldX, worldY) => {
    ...
    const processBlockDither = async () => {
        ...
        for (let yb = y0, rowIdx = 0; yb <= yMax; yb += cellSize, rowIdx++) {
            ...
            const processCell = (cx) => {
                ...
                if (!serpentine) {
                    if (cx + 1 < cellsAcross)
                        cErrCurr[cx + 1] += err * 0.5;
                    if (cx - 1 >= 0)
                        cErrNext[cx - 1] += err * 0.25;
                }
                else {
                    if (cx - 1 >= 0)
                        cErrCurr[cx - 1] += err * 0.5;
        ...
        for (let row = coverage.minRow; row <= coverage.maxRow; row++) {
            ...
            const processPixel = (x) => {
                ...
                if (params.ditherEnabled && ditherStrength > 0 && errCurr && errNext) {
                    ...
                    const err = (frac - q) * ditherStrength;
                    if (localX + 1 < bboxW)
                        errCurr[localX + 1] += err * 0.5;
                    if (localX - 1 >= 0)
                        errNext[localX - 1] += err * 0.25;
...
async function fillConcentricToBuffer(params, hooks) {
    const width = Math.max(1, Math.floor(params.bbox.width));
    const height = Math.max(1, Math.floor(params.bbox.height));
