...
- Pointer move handlers also call `setCursorPosition` every event (`src/hooks/canvas/handlers/pointerHandlers.ts`) to keep the cursor in sync without triggering React re-render loops on the hot path.
