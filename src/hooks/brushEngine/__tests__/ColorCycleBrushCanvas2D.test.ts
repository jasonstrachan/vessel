...
describe('ColorCycleBrushCanvas2D', () => {
  ...
  it('uses an authored linear gradient span without changing direction-only fills', async () => {
    const vertices = [
      { x: 0, y: 0 },
      { x: 7, y: 0 },
      { x: 7, y: 5 },
    ...
    const fillWithSpan = async (span?: number): Promise<Uint8Array> => {
      const canvas = makeCanvas();
      const brush = new ColorCycleBrushCanvas2D(canvas);
      brush.setGradientBands(16);
      brush.setDitherEnabled(false);
      const layerId = `layer-${span ?? 'shape-fit'}`;

      await brush.fillShapeLinear(vertices, { x: 1, y: 0 }, layerId, 1, {
        continuous: true,
        ccGradient: true,
        linearGradientSpan: span,
      });

      const animator = (brush as any).getAnimator(layerId);
      return animator?.indexBuffer as Uint8Array;
    };

    const shortSpan = await fillWithSpan(4);
    const longSpan = await fillWithSpan(12);
    const shapeFittedSpan = await fillWithSpan(8);
    const legacyDirectionOnly = await fillWithSpan();
    const leftInteriorPixel = 2 * 8 + 1;
    const rightInteriorPixel = 2 * 8 + 6;

    expect(shortSpan[leftInteriorPixel]).toBeLessThan(longSpan[leftInteriorPixel]);
    expect(shortSpan[rightInteriorPixel]).toBeGreaterThan(longSpan[rightInteriorPixel]);
    expect(Array.from(legacyDirectionOnly)).toEqual(Array.from(shapeFittedSpan));
  });

  it('uses an authored linear gradient span for perceptual dither fills', async () => {
    const fillMathMocks = jest.requireMock('@/utils/colorCycle/fillMath') as {
      applyEdgePadding: jest.Mock;
    };
    const previousApplyEdgePadding = fillMathMocks.applyEdgePadding.getMockImplementation();
    fillMathMocks.applyEdgePadding.mockImplementation((value: number) => value);
    ...
    const fillWithSpan = async (span: number): Promise<Uint8ClampedArray> => {
      const canvas = makeCanvas();
      const brush = new ColorCycleBrushCanvas2D(canvas);
      brush.setDitherEnabled(true);
      brush.setPerceptualDither(true);
      ...
      await brush.fillShapeLinear(vertices, { x: 1, y: 0 }, `layer-${span}`, 1, {
        continuous: true,
        linearGradientSpan: span,
      });

      return new Uint8ClampedArray(
        (fillDitherMocks.fillLinear.mock.calls[0]?.[0] as ImageData).data,
      );
    };
    ...
    try {
      ...
      expect(Array.from(shortSpan)).not.toEqual(Array.from(longSpan));
    } finally {
      fillMathMocks.applyEdgePadding.mockImplementation(
        previousApplyEdgePadding ?? (() => undefined),
      );
    }
  });
