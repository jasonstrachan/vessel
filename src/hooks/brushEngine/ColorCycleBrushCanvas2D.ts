...
export class ColorCycleBrushCanvas2D {
  ...
  /**
   * Fill shape with circular gradient centered on polygon centroid.
   * Uses the polygon as a mask so only the user-drawn shape is filled.
   */
  fillShapeCircular(
    ...
    if (!layerId) {
      throw new Error('fillShapeCircular requires a layerId');
    }

    if (!vertices || !Array.isArray(vertices)) {
      console.warn('Invalid vertices provided to fillShapeCircular');
      return;
    }

    if (vertices.length < 3) {
      console.warn('fillShapeCircular requires at least 3 vertices');
      return;
    }
    ...
    if (!isFinite(maxRadius) || maxRadius < 1e-3) {
      maxRadius = Math.max(1, Math.max(maxX - minX, maxY - minY) / 2);
    }
    const safeRadius = Math.max(1, maxRadius);
    ...
    const clamp01 = (value: number) => Math.min(1, Math.max(0, value));
    const normalizedAt = (sampleX: number, sampleY: number): number => {
      ...
      const normalized = dist / safeRadius;
        ...
        if (this.ditherEnabled && cellSize > 1) {
          ...
          const processCell = (cx: number) => {
            ...
            if (cached < 0) {
              ...
              const sampleX = xCenter + 0.5;
              const sampleY = yCenterBlock + 0.5;
              let r = normalizedAt(sampleX, sampleY);
              if (this.ditherEnabled) {
                const jitterScale = 0.35;
                const quantLevels = Math.max(2, numBands);
                const j = (noiseAt(xCenter, yCenterBlock) - 0.5) * (jitterScale / quantLevels);
                r = clamp01(r + j);
              }
          ...
          if (!serpentine) {
            for (let x = startX; x <= endX; x++) {
              let r = normalizedAt(x + 0.5, y + 0.5);
              if (this.ditherEnabled) {
                const jitterScale = 0.35;
                const j = (noiseAt(x, y) - 0.5) * (jitterScale / quantLevels);
                r = clamp01(r + j);
          ...
          const quantLevels = Math.max(2, this.gradientBands || 12);
          for (let x = startX; x <= endX; x++) {
            const r = normalizedAt(x + 0.5, y + 0.5);
            const scaled = r * quantLevels;
            const k = Math.min(quantLevels - 1, Math.floor(scaled));
            const pos = k / quantLevels;
            const outIdx = indexFromNormalized(pos);
            this.logSetIndexSample(id, x, y);
            animator.setIndex(x, y, outIdx);
          }
