...
export const fillCcGradientDither = async ({
  ...
  if (flatCycle) {
    ...
    if (algorithm === 'sierra-lite') {
      ...
      const diversity = clamp01((ditherPatternDiversity ?? 100) / 100);
      const bits = resolveSierraLiteBinaryField({
        ...
        phaseX,
        phaseY,
        identityKey: wrappedBaseOffset,
        lowKey: flatCycleLowLut[128] ?? 0,
        highKey: flatCycleHighLut[128] ?? 1,
        diversity,
      });
      bits.forEach((bit, cellIdx) => {
        if (!activeMask[cellIdx]) return;
        const coverage = cellCoverage[cellIdx] ?? 0;
        cellIndices[cellIdx] = bit === 1
    ...
    if (algorithm in ERROR_DIFFUSION_KERNELS) {
      ...
      const profile = ERROR_DIFFUSION_PROFILES[algorithm as keyof typeof ERROR_DIFFUSION_PROFILES];
      const errBuf = new Float32Array(gridW * gridH);
      const jitterScale = algorithm === 'sierra-lite' ? 0 : profile.jitterBase * 0.25;

      for (let cy = 0; cy < gridH; cy += 1) {
        const activeRow = activeCellsByRow[cy];
        if (!activeRow.length) continue;

        const useSerpentine = algorithm !== 'sierra-lite' && kernel.serpentine !== false;
        const leftToRight = useSerpentine ? (cy & 1) === 0 : true;
        const start = leftToRight ? 0 : activeRow.length - 1;
        const end = leftToRight ? activeRow.length : -1;
        const step = leftToRight ? 1 : -1;
        ...
        for (let i = start; i !== end; i += step) {
          ...
          let local = clamp01(initialLocal);
          if (jitterScale > 0) {
    ...
    for (let cy = 0; cy < gridH; cy += 1) {
      ...
      for (let i = 0; i < activeRow.length; i += 1) {
        ...
        const chooseUpper = lower < clampedLevels - 1 && adj >= 0.5;
