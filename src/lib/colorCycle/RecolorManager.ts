...
export class RecolorManager {
  ...
  async processLayer(layer: Layer, options: RecolorOptions = {}): Promise<boolean> {
    try {
      ...
      logError(`[RecolorManager] Failed to process layer ${layer.id}:`, error);
      return false;
    }
  }
  ...
  async extractColors(layer: Layer, options: ExtractColorsOptions): Promise<Array<{ position: number; color: string }> | null> {
    try {
      ...
      logError('[RecolorManager] Color extraction failed:', error);
  ...
  analyzeLayerColors(layer: Layer): ColorAnalysis | null {
    ...
    try {
      ...
      logError('[RecolorManager] Color analysis failed:', error);
      return null;
    }
  }
  ...
  async registerExistingLayer(layer: Layer): Promise<boolean> {
    try {
      ...
      debugWarn('raw-console', '[RecolorManager] registerExistingLayer failed:', e);
      return false;
    }
  }
  ...
  updateGradient(layer: Layer, gradient: Array<{ position: number; color: string }>): boolean {
    try {
      ...
      logError('[RecolorManager] Failed to update gradient:', error);
      return false;
    }
  }
  ...
  setPaletteDirectionalOrder(layerId: string, angleDeg: number): boolean {
    try {
      ...
      debugWarn('raw-console', '[RecolorManager] Failed to set palette directional order', e);
      return false;
    }
  }
  ...
  autoSetAnimationDirection(layerId: string, angleDeg: number): 'forward' | 'reverse' | null {
    try {
      ...
      debugWarn('raw-console', '[RecolorManager] Failed to auto set animation direction', e);
      return null;
    }
  }
  ...
  clearPaletteDirectionalOrder(layerId: string): boolean {
    try {
      ...
      debugWarn('raw-console', '[RecolorManager] Failed to clear palette directional order', e);
      return false;
    }
  }
  ...
  convertToNormal(layer: Layer): boolean {
    try {
      ...
      debugLog('raw-console', `[RecolorManager] Converted layer ${layer.id} back to normal`);
      ...
      logError(`[RecolorManager] Failed to convert layer ${layer.id} to normal:`, error);
      return false;
    }
  }
  ...
  cleanup(layer: Layer): void {
    if (layer.layerType === 'color-cycle' && layer.colorCycleData?.mode === 'recolor') {
      this.animationController.unregisterLayer(layer.id);
      debugLog('raw-console', `[RecolorManager] Cleaned up resources for layer ${layer.id}`);
    }
  }
  ...
  cleanupAll(): void {
    ...
    debugLog('raw-console', '[RecolorManager] Cleaned up all resources');
  }
